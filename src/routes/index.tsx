...
.gs-root .slide-layer{position:relative;height:175vh;background:var(--cream);z-index:1}
.gs-root .slide-layer>section,.gs-root .slide-layer>header{position:sticky;top:0;width:100%;height:100vh;min-height:100vh;display:flex;align-items:center;overflow:hidden;background:var(--cream);box-shadow:0 -24px 70px -48px rgba(49,28,16,.45)}
...
/* Section heads */
.gs-root .sec-head{text-align:center;max-width:42rem;margin:0 auto 32px}
...
/* FEATURES */
.gs-root .features{padding:54px 0 46px;position:relative}
...
.gs-root .feat-steps{display:flex;flex-direction:column;gap:14px}
.gs-root .fstep{background:linear-gradient(165deg,#fff,#fff8f1);border:1px solid var(--line);border-radius:24px;padding:18px 22px;display:flex;gap:18px;align-items:center;box-shadow:var(--shadow-sm);transition:transform .3s,box-shadow .3s,border-color .3s;cursor:pointer;position:relative}
...
@media(max-width:960px){
  ...
  .gs-root .slide-layer{height:auto;min-height:auto;display:block;box-shadow:none}
  .gs-root .slide-layer>section,.gs-root .slide-layer>header{position:relative;height:auto;min-height:auto;display:block;overflow:visible}
